...
numericalDeriv :: (RealFrac a, Show a) => a -> Objective a -> a -> a -> a
...
newton :: (RealFrac a, Show a) => Objective a -> Derivative a -> a -> [a]
newton f fDeriv x0 =
    let x = x0 - f x0 / fDeriv x0
    in x : newton f fDeriv x

findRoot' :: (RealFrac a, Show a) => Int -> a -> Objective a -> Derivative a -> a -> a
